...
public abstract class AI {
	...
	protected void flee(Creature hunter) {
		...
		if(Engine.getGame().getAtlas().getCurrentZone().getCreature(p) == null) {
	...
	private boolean open(Point p) {
		Door door = null;
		for(long uid : Engine.getGame().getAtlas().getCurrentZone().getItems(p)) {
			if(Engine.getGame().getEntity(uid) instanceof Door) {
				door = (Door)Engine.getGame().getEntity(uid);
			}
		}
		if(door != null) {
	...
	protected void wander() {
		...
		if(Engine.getGame().getAtlas().getCurrentZone().getCreature(p) == null && !player.equals(p)) {
	...
	protected void wander(Point destination) {
		...
		if(Engine.getGame().getAtlas().getCurrentZone().getCreature(next) == null && !player.equals(next)) {
	...
	protected void hunt(Creature prey) {
		...
		if(p.distance(preyPos.x, preyPos.y) < 1) {
			...
			Weapon weapon = (Weapon)Engine.getGame().getEntity(uid);
			if(creature.getInventoryComponent().hasEquiped(Slot.WEAPON) && weapon.isRanged()) {
				if(!(CombatUtils.getWeaponType(creature).equals(WeaponType.THROWN) || equip(Slot.AMMO))) {
					InventoryHandler.unequip(weapon, creature);
				}
			} else if(!creature.getInventoryComponent().hasEquiped(Slot.WEAPON)) {
				equip(Slot.WEAPON);
			} 
			Engine.post(new CombatEvent(creature, prey));
		} else if(Engine.getGame().getAtlas().getCurrentZone().getCreature(p) == null) {
