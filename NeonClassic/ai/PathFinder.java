...
public class PathFinder {
		...
		while(!todo.isEmpty() && i-- > 0) {
			...
			for(Point neighbour : neighbours(next)) {
				if(neighbour.equals(to)) {
					...
					next = null;
					break;
				} else if(Engine.getGame().getAtlas().getCurrentZone().getRegion(neighbour).getMovMod() == Region.Modifier.BLOCK) {
	...
	private static int terrainPenalty(Point neighbour) {
		// betere modifiers?
		switch(Engine.getGame().getAtlas().getCurrentZone().getRegion(neighbour).getMovMod()) {
	...
	private static int doorPenalty(Point neighbour) {
		for(long uid : Engine.getGame().getAtlas().getCurrentZone().getItems(neighbour)) {
			if(Engine.getGame().getEntity(uid) instanceof Door) {
				Door door = (Door)Engine.getGame().getEntity(uid);
