...
public class JVectorPane extends JComponent {
	public static final int DEFAULT_ZOOM = 15;	// default zoom level
		...
		synchronized(renderables) {
			...
			for(Renderable r : renderables) {
				if(r instanceof Item) {
					ItemRenderer.paint((Item)r, buffer, zoom, selection.contains(r));
				} else if(r instanceof Creature) {
					CreatureRenderer.paint((Creature)r, buffer, zoom, selection.contains(r));
				} else if(r instanceof Region) {
					RegionRenderer.paint((Region)r, buffer, zoom, selection.contains(r));
				} else if (r instanceof JVShape) {
					((JVShape)r).paint(buffer, zoom, selection.contains(r));
				}
			}
			buffer.dispose();
