...
public class DungeonGenerator {
	...
	private UIDStore store = Engine.getGame().getStore();
	private Atlas atlas = Engine.getGame().getAtlas();
	...
	public void generate(Door door, Zone previous) {
		...
		Door tdoor = (Door)EntityFactory.getItem(doorType, p.x, p.y, store.createNewEntityUID());
		store.addEntity(tdoor);
			...
			for(int to : connections) {
				...
				if(to != previous.getIndex()) {
					...
					Door toDoor = (Door)EntityFactory.getItem(theme.doors.split(",")[0], 
							pos.x, pos.y, store.createNewEntityUID());
					store.addEntity(toDoor);
					...
					for(long uid : previous.getItems()) {
						if(store.getEntity(uid) instanceof Door) {
							Door fromDoor = (Door)store.getEntity(uid);
...
//				case ENTRY: zone.addRegion(new Region("wall_adobe", x, y, 1, 1, null, layer + 1)); break;
	...
	private void addDoor(String terrain, String id, int x, int y, int layer) {
		Door door = (Door)EntityFactory.getItem(id, x, y, store.createNewEntityUID());
		store.addEntity(door);
	...
	private void addCreature(String description, int x, int y) {
		...
		Creature creature = EntityFactory.getCreature(id, x, y, store.createNewEntityUID());
	...
	private void addItem(String description, int x, int y) {
		...
		Item item = EntityFactory.getItem(id, x, y, store.createNewEntityUID());
		store.addEntity(item);
		if(item instanceof Container) {
			for(String s : ((RItem.Container)item.resource).contents) {
				Item i = EntityFactory.getItem(s, store.createNewEntityUID());
