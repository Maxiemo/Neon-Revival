...
public class WildernessGenerator {
		...
		if(!isOnTop(region, regions)) {	// als er nog regions boven deze region liggen
			...
			if(region.y > 0) {	// bovenkant van map
				for(int i = 0; i < region.width; i++) {
					terrain[0][i+1] = zone.getRegion(new Point(region.x + i, region.y - 1)).getTerrainType();
				}
			}
			if(region.y + region.height < zone.getHeight() - 1) {	// onderkant
				for(int i = 0; i < region.width; i++) {
					terrain[region.height + 1][i+1] = 
							zone.getRegion(new Point(region.x + i, region.y + region.height)).getTerrainType();
				}
			}
			if(region.x > 0) {	// links
				for(int i = 0; i < region.height; i++) {
					terrain[i+1][0] = zone.getRegion(new Point(region.x - 1, region.y + i)).getTerrainType();
				}
			}
			if(region.x + region.width < zone.getWidth() - 1) {	// rechts
				for(int i = 0; i < region.height; i++) {
					terrain[i+1][region.width + 1] = 
							zone.getRegion(new Point(region.x + region.width, region.y + i)).getTerrainType();
				}
			}
					
			// terrain genereren
			generateTerrain(region.width, region.height, theme, region.getTerrainType());
	...
	private void decompose(Region region, Collection<Region> regions, RRegionTheme theme) {
		...
		while(i < 5 && !area.isEmpty()) {
			...
			for(Rectangle r : pieces) {
				if(area.contains(r)) {
					RTerrain rt = (RTerrain)Engine.getResources().getResource(region.getTerrainType(), "terrain");
					zone.addRegion(new Region(region.getTerrainType(), r.x, r.y, r.width, r.height, theme, region.z, rt));
	...
	private void divide(Region region, RRegionTheme theme) {
		String texture = region.getTerrainType();
		...
		for(String id : theme.creatures.keySet()) {
			for(int i = (int)(Dice.roll(1, theme.creatures.get(id), 0)*ratio); i > 0; i--) {
				...
				Creature creature = EntityFactory.getCreature(id, rx + x, ry +y, 
						Engine.getGame().getStore().createNewEntityUID());
				...
				if(terrain.modifier == Region.Modifier.SWIM && 
					...
					Engine.getGame().getStore().addEntity(creature);
...
//					System.out.println(terrain[j+1][i+1]);
					...
					for(String entry : data) {
						if(entry.startsWith("i:")) {
							...
							long uid = Engine.getGame().getStore().createNewEntityUID();
							Item item = EntityFactory.getItem(id, region.x + i, region.y + j, uid);
							Engine.getGame().getStore().addEntity(item);
							if(item instanceof Container) {
								for(String s : ((RItem.Container)item.resource).contents) {
									Item content = EntityFactory.getItem(s, Engine.getGame().getStore().createNewEntityUID());
									((Container)item).addItem(content.getUID());
									Engine.getGame().getStore().addEntity(content);
								}
							}
							zone.addItem(item);
							...
							Engine.getGame().getStore().addEntity(creature);
							zone.addCreature(creature);							
						} else if(!entry.isEmpty() && !entry.equals(region.getTerrainType())) {
