...
public class UIDStore {	
	...
	/**
	 * Tells this UIDStore to use the given MapDB cache.
	 * 
	 * @param file
	 */
	public UIDStore(String file) {
		db = DBMaker.newFileDB(new File(file)).make();
		objects = db.createHashMap("objects").valueSerializer(new EntitySerializer()).makeOrGet();
		mods = db.createHashMap("mods").makeOrGet();
	}
	
	/**
	 * @return	the mapdb cache used by this UIDStore
	 */
	public DB getCache() {
		return db;
	}
