...
public class CombatHandler {
	@Subscribe public void handleCombat(CombatEvent ce) {
		if(!(ce instanceof CombatEvent.Finished)) {
			int result = 0;
			switch(ce.getType()) {
			...
			Engine.post(new CombatEvent.Finished(ce.getAttacker(), ce.getDefender(), result));
		}
	}
	...
	private int shoot(Creature shooter, Creature target) {
		...
		Weapon ammo = (Weapon)Engine.getGame().getEntity(shooter.getInventoryComponent().get(Slot.AMMO));
		InventoryHandler.removeItem(shooter, ammo);
	...
	private int fling(Creature thrower, Creature target) {
		Weapon weapon = (Weapon)Engine.getGame().getEntity(thrower.getInventoryComponent().get(Slot.AMMO));
		InventoryHandler.removeItem(thrower, weapon);
