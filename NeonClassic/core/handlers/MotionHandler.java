...
public class MotionHandler {
	...
	public static byte teleport(Creature creature, Door door) {
		...
		Atlas atlas = game.getAtlas();
		if(door.portal.isPortal()) {
			Zone previous = atlas.getCurrentZone();	// effe huidige zone bufferen
			if(door.portal.getDestMap() != 0) {
				// effen map laden en deur terug laten verwijzen
				Map map = atlas.getMap(door.portal.getDestMap());
				Zone zone = map.getZone(door.portal.getDestZone());
				for(long uid : zone.getItems(door.portal.getDestPos())) {
					Entity i = Engine.getGame().getEntity(uid);
					if(i instanceof Door) {
						((Door)i).portal.setDestMap(atlas.getCurrentMap());
					}
				}
				game.setMap(map);
				door.portal.setDestMap(atlas.getCurrentMap());
			...
			for(long uid : atlas.getCurrentZone().getItems(bounds)) {
				Entity i = Engine.getGame().getEntity(uid);
				if(i instanceof Door) {
					((Door)i).lock.open();
				}
	...
	public static byte move(Creature actor, Point p) {
		Region region = Engine.getGame().getAtlas().getCurrentZone().getRegion(p);
		if(p == null || region == null) {
			return NULL;
		}
		
		// effen kijken of er geen gesloten deur aanwezig is
		Collection<Long> items = Engine.getGame().getAtlas().getCurrentZone().getItems(p);
		for(long uid : items) {
			Entity i = Engine.getGame().getEntity(uid);
			if(i instanceof Door) {
