...
public class InventoryHandler {
	 ...
	 * @param event
	 */
	@Subscribe public void handle(StoreEvent event) {
		switch(event.getMode()) {
		case ADD:
			Engine.getGame().getStore().addEntity(event.getEntity());
			break;
		case REMOVE:
			Engine.getGame().getStore().removeEntity(event.getUID());
			break;
		}
	}
	 ...
	 */
	public static void addItem(Creature creature, Item item) {
		if(item instanceof Item.Coin) {
			creature.getInventoryComponent().addMoney(item.resource.cost);
		} else {
			creature.getInventoryComponent().addItem(item.getUID());
		}
	}

	/**
	 * Adds an item with the given id to a creature's inventory.
	 * 
	 * @param creature
	 * @param id
	 */
	public static void addItem(Creature creature, String id) {
		Item item = EntityFactory.getItem(id, Engine.getGame().getStore().createNewEntityUID());
		Engine.getGame().getStore().addEntity(item);
		addItem(creature, item);
	}
	...
	public static void removeItem(Creature creature, Item item) {
		if(creature.getInventoryComponent().hasEquiped(item.getUID())) {
			unequip(item, creature);	// eerst unequipen als ge dit nog aanhebt
		}
		creature.getInventoryComponent().removeItem(item.getUID());
	}
	...
	public static void unequip(Item item, Creature creature) {
		Inventory inventory = creature.getInventoryComponent();
//		Item item = (Item)Engine.getStore().getEntity(uid);
	...
	public static int getWeight(Creature creature) {
		float sum = 0;
		for(long uid : creature.getInventoryComponent()) {
			sum += ((Item)Engine.getGame().getEntity(uid)).resource.weight;
		}
